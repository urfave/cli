...
Go already removes unreachable code during compilation, so the first step is to
...
Build the same command with stripped symbol and debug information to compare the
...
open an issue with the Go version, build command, a minimal reproduction, and
the `go tool nm -size` output that shows the largest symbols.
